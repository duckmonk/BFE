...
import { Typography, Box, Button, Grid, TextField, Select, MenuItem, Table, TableBody, TableCell, TableContainer, TableHead, TableRow, Paper, IconButton, Dialog, DialogTitle, DialogContent, DialogActions, Snackbar, Alert } from '@mui/material';
...
export default function InquiryDashboard() {
  ...
  // 添加复制日历链接功能
  const handleCopyCalendar = (inquiryId: number) => {
    const calendarUrl = `${window.location.origin}/schedule?inquiryId=${inquiryId}`;
    ...
    if (navigator.clipboard && window.isSecureContext) {
      ...
      console.log("Clipboard API is not supported or insecure context");
      fallbackCopyToClipboard(calendarUrl);
    }
  };
          ...
          <Table size="small">
            <TableHead>
              <TableRow>
                ...
                <TableCell align="center" sx={{ minWidth: '200px' }}>Calendar</TableCell>
                ...
                inquiries.map((row) => (
                  <TableRow key={row.id}>
                    ...
                    <TableCell align="center" sx={{ minWidth: '200px' }}>
                      {row.sendOutCalendarDateInq ? (
                        <Typography>
                          {formatTimestamp(row.sendOutCalendarDateInq)}
                        </Typography>
                      ) : (
                        <Button
                          variant="contained"
                          onClick={() => handleCopyCalendar(row.id)}
                          fullWidth
